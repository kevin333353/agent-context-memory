...
try {
  ...
  $expectedVersion = (Get-Content -Raw -Encoding UTF8 -LiteralPath (Join-Path $Root "VERSION")).Trim()
  Assert-True ($cliVersion.Stdout.Trim() -eq $expectedVersion) "cli version did not report $expectedVersion"
